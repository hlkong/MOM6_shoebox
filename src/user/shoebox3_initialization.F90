...
!> Differences from shoebox2: 
!> removed Scotia Arc and Patagonia in the Southern Ocean;
!> so that Southern Ocean becomes flat-bottomed and totally reentrant
...
#include "version_variable.h"
  ...
  latext = G%len_lat
  lonext = G%len_lon
  reentrantn = 35.0/latext         ! non-dimensional northern bound of the reentrant zone
  D = 0.0
  ...
  do i=is,ie                ! zonal grid points
    ...
    D(i,j) = 1.0 -spike(y, ll/latext) & ! Antarctica
              -spike(1.0-y, ll/latext) & ! northern wall
              -spike(x-dx/2, ll/lonext)*homo(y-(1.+reentrantn)/2,(1-reentrantn)/2) & ! West boundary
              -spike(x-1+dx/2, ll/lonext)*homo(y-(1.+reentrantn)/2,(1-reentrantn)/2) ! East boundary
