!> Wind and buoyancy forcing for the shoebox7 configurations
!> difference from shoebox9: 
!>  1) no wind stress outside of the Southern Ocean (north of 30S)
!>  2) no buoyancy forcing module

module shoebox7_surface_forcing
...
public shoebox7_buoyancy_forcing
public shoebox7_wind_forcing
public shoebox7_surface_forcing_init

!> This control structure should be used to store any run-time variables
!! associated with the shoebox7 forcing.  It can be readily modified
...
type, public :: shoebox7_surface_forcing_CS ; private
  ...
  logical :: first_call = .true. !< True until shoebox7_buoyancy_forcing has been called
end type shoebox7_surface_forcing_CS

contains


subroutine shoebox7_buoyancy_forcing(sfc_state, fluxes, day, dt, G, CS)
  ...
  type(shoebox7_surface_forcing_CS), pointer  :: CS !< Control structure for this module.
  ! Local variables
  ...
  if (CS%use_temperature) then
    call MOM_error(FATAL, "shoebox7_buoyancy_forcing: " // &
...
! set up target surface density profile
  ...
  ! compute the buoyancy flux needed to restore current density to target
  ! density
  if (CS%restorebuoy) then
    if (CS%use_temperature) then      ! if temperature is used to restore buoyancy
      call MOM_error(FATAL, "shoebox7_buoyancy_surface_forcing: " // &
...
end subroutine shoebox7_buoyancy_forcing



!> Sets the surface wind stresses, forces%taux and forces%tauy for the
!! shoebox7 forcing configuration.
subroutine shoebox7_wind_forcing(sfc_state, forces, day, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface
                                                    !state of the ocean.
...
end subroutine shoebox7_wind_forcing
...
!> Initializes the shoebox7 control structure.
subroutine shoebox7_surface_forcing_init(Time, G, param_file, diag, CS)
  ...
  type(shoebox7_surface_forcing_CS), pointer :: CS !< A pointer that is set to point to the control structure
                                                    !! for this module
  ! This include declares and sets the variable "version".
#include "version_variable.h"
  ! Local variables
  character(len=40)  :: mod = "shoebox7_surface_forcing" ! This module's name.

  if (associated(CS)) then
    call MOM_error(WARNING, "shoebox7_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
end subroutine shoebox7_surface_forcing_init
...
end module shoebox7_surface_forcing
